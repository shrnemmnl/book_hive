...
from users.models import (
    CustomerSupport, CustomUser, Order, OrderItem, Review,
    Transaction, Wallet
)
from .models import Coupon, CouponUsage, Genre, Product, ProductImage, Variant
...
def admin_dashboard(request):
    ...
    if filter_type in ['today', 'week', 'month', 'year']:
        ...
        chart_labels = [item['day'].strftime(
            '%d %b') if item['day'] else '' for item in daily_sales]
...
def update_order_item_status(request, order_item_id):
    ...
    if old_status == 'pending' and status != 'pending':
        ...
        try:
            ...
            logger.warning(
                f"Could not generate invoice for order {
                    order_item.order.order_id}: {
                    str(e)}. Make sure migrations are run.")
...
def add_new_book(request):
    if request.method == 'POST':
        ...
        if not image:
            ...
            # Check MIME type
            if is_valid:
                file_mime_type = image.content_type.lower() if hasattr(
                    image, 'content_type') and image.content_type else ""
                if not file_mime_type or file_mime_type not in valid_mime_types:
                    error[
                        'image'] = "Invalid file type detected. Please upload a valid image file (jpg, jpeg, png, gif, or webp formats only)."
                    is_valid = False
        ...
        if not is_valid:
            ...
            return render(request, 'admin/add_new_book.html',
                          {'all_genres': all_genres, 'error': error})
        ...
        return render(request, 'admin/add_new_book.html',
                      {'redirect': True, 'redirect_url': 'books'})
    ...
    if request.method == 'POST':
        ...
        if image_cropped:
            ...
            if is_valid:
                ...
                if not file_mime_type or file_mime_type not in valid_mime_types:
                    error[
                        'valid_image'] = "Invalid file type detected. Please upload a valid image file (jpg, jpeg, png, gif, or webp formats only)."
                    is_valid = False
        ...
        if not is_valid:
            return render(request, 'admin/book_edit.html',
                          {'book': book, 'genres': genres, 'error': error})
        ...
        if not image1:
            ...
            if is_valid:
                ...
                if not file_extension or file_extension not in valid_extensions:
                    ...
                    is_valid = False
            if is_valid:
                file_mime_type = image1.content_type.lower() if hasattr(
                    image1, 'content_type') and image1.content_type else ""
                if not file_mime_type or file_mime_type not in valid_mime_types:
                    error['image1'] = "Image 1: Invalid file type. Please upload images only (jpg, jpeg, png, gif, or webp formats)."
                    is_valid = False
        ...
        if not image2:
            ...
            if is_valid:
                ...
                if not file_extension or file_extension not in valid_extensions:
                    ...
                    is_valid = False
            if is_valid:
                file_mime_type = image2.content_type.lower() if hasattr(
                    image2, 'content_type') and image2.content_type else ""
                if not file_mime_type or file_mime_type not in valid_mime_types:
                    error['image2'] = "Image 2: Invalid file type. Please upload images only (jpg, jpeg, png, gif, or webp formats)."
                    is_valid = False
        ...
        if not image3:
            ...
            if is_valid:
                ...
                if not file_extension or file_extension not in valid_extensions:
                    ...
                    is_valid = False
            if is_valid:
                file_mime_type = image3.content_type.lower() if hasattr(
                    image3, 'content_type') and image3.content_type else ""
                if not file_mime_type or file_mime_type not in valid_mime_types:
                    error['image3'] = "Image 3: Invalid file type. Please upload images only (jpg, jpeg, png, gif, or webp formats)."
                    is_valid = False
...
def user_search(request):
    ...
    users = CustomUser.objects.filter(
        first_name__istartswith=search_query).exclude(
        is_superuser=True).annotate(
            query_count=Count('support_queries')).order_by(
                '-query_count',
        '-date_joined')
...
def variant_edit(request, variant_id):
    ...
    return render(request, 'admin/variant_edit.html',
                  {'variant': variant, 'images': images, 'error': error})
...
def admin_order_details(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if new_status == 'delivered' and order.payment_method == 'cod':
            ...
            if not existing_transaction:
                ...
                item_product_discount = (Decimal(str(order_item.unit_price)) -
                                         Decimal(str(order_item.discount_price))) * order_item.quantity
                ...
                if order.coupon_discount and order.coupon_discount > 0:
                    ...
                    if total_items > 0:
                        coupon_discount_per_item = Decimal(
                            str(order.coupon_discount)) / total_items
                ...
                Transaction.objects.create(
                    ...
                    description=f'COD payment for item {
                        order_item.product_variant.product.book_title} from order {
                        order.order_id}',
                    payment_method='cod',
                    status='completed')

            # Mark order as paid when any item is delivered (payment received upon
            # delivery for COD)
            if not order.is_paid:
                ...
                messages.success(
                    request, f"Order payment status updated to Paid (COD payment received upon delivery).")
                logger.info(
                    f"Order {
                        order.order_id} marked as paid after item {
                        order_item.id} delivery (COD payment method)")
        ...
        if old_status == 'request to return' and new_status == 'return approved':
            ...
            if order.is_paid:
                user = order.user
                # Calculate accurate refund amount (gross - product discount -
                ...
                messages.success(
                    request, f"Refunded ₹{
                        item_refund_amount:.2f} to user {
                        user.email}'s wallet for item {
                        order_item.product_variant.product.book_title}.")
                ...
                try:
                    ...
                    if Invoice.objects.filter(order=order).exists():
                        credit_note = generate_credit_note_for_order_item(
                            order_item, reason=order_item.cancel_reason or 'Return approved')
                        if credit_note:
                            logger.info(
                                f"Credit note {
                                    credit_note.credit_note_number} generated for returned item {
                                    order_item.id}")
                except Exception as e:
                    logger.warning(
                        f"Could not generate credit note for item {
                            order_item.id}: {
                            str(e)}")

        # If item is being cancelled/returned and order is paid, handle refund
        # (for direct cancellations, not from request)
            ...
            try:
                Transaction.objects.create(
                    ...
                    transaction_type='refund',
                    amount=item_refund_amount,
                    description=f'Refund for order item {
                        order_item.id} ({
                        order_item.product_variant.product.book_title}) from order {
                        order.order_id}',
                    payment_method='wallet',
            ...
            try:
                ...
                if Invoice.objects.filter(order=order).exists():
                    credit_note = generate_credit_note_for_order_item(
                        order_item, reason=order_item.cancel_reason or 'Cancelled/Returned')
                    if credit_note:
                        logger.info(
                            f"Credit note {credit_note.credit_note_number} generated for cancelled/returned item {order_item.id}")
            except Exception as e:
                logger.warning(
                    f"Could not generate credit note for item {
                        order_item.id}: {
        ...
        if old_status == 'request to cancel' and new_status == 'cancelled':
            ...
            if order.is_paid and not refund_message_shown:
                ...
                try:
                    ...
                    if Invoice.objects.filter(order=order).exists():
                        credit_note = generate_credit_note_for_order_item(
                            order_item, reason=order_item.cancel_reason or 'Cancellation approved')
                        if credit_note:
                            logger.info(
                                f"Credit note {
                                    credit_note.credit_note_number} generated for cancelled item {
                                    order_item.id}")
                except Exception as e:
                    logger.warning(
                        f"Could not generate credit note for item {
        ...
        # If item is being cancelled directly (not from request) and order is not
        # paid (COD), just update stock
    ...
    for item in order.order_items.all():
        ...
        item_product_discount = (Decimal(str(item.unit_price)) -
                                 Decimal(str(item.discount_price))) * item.quantity
...
def sales_report(request):
    ...
    for item in refunded_items:
        ...
        # Product/Genre Discount = (unit_price - discount_price) * quantity
        product_disc = (Decimal(str(item.unit_price)) -
                        Decimal(str(item.discount_price))) * item.quantity
    ...
    for item in order_items_page:
        ...
        # Product/Genre Discount = (unit_price - discount_price) * quantity
        product_discount = (Decimal(str(item.unit_price)) -
                            Decimal(str(item.discount_price))) * item.quantity
        ...
        if item.order.coupon_discount and item.order.coupon_discount > 0:
            ...
            if total_items_in_order > 0:
                coupon_discount_per_item = Decimal(
                    str(item.order.coupon_discount)) / total_items_in_order
...
def download_sales_report_pdf(request):
    ...
    total_net_amount_from_items = Decimal('0.00')
    order_items_all = OrderItem.objects.filter(order__in=orders).select_related(
        'order', 'product_variant', 'product_variant__product').prefetch_related('order__order_items')
    ...
    order_items = OrderItem.objects.filter(
        order__in=orders).select_related(
        'order',
        'order__user',
        'product_variant',
        'product_variant__product').prefetch_related('order__order_items').order_by(
            '-order__created_at',
            '-id')[
                :200]  # Limit to 200 items for PDF
    ...
    # repeatRows=1 to repeat header on each page
    order_items_table = Table(order_items_data, colWidths=col_widths, repeatRows=1)
    ...
    response['Content-Disposition'] = f'attachment; filename="sales_report_{
...
def download_sales_report_excel(request):
    ...
    for item in order_items:
        ...
        ws.cell(row=row_num, column=2, value=item.order.created_at.strftime(
            '%Y-%m-%d %H:%M')).border = border
        ws.cell(
            row=row_num, column=3, value=f"{
                item.order.user.first_name} {
                item.order.user.last_name}").border = border
...
def admin_transactions(request):
    ...
    transactions = Transaction.objects.select_related(
        ...
        'order_item__product_variant__product').all().order_by('-transaction_date')
...
def transaction_detail(request, order_id):
    ...
    if transaction.order_item:
        ...
        if transaction.order and transaction.order.coupon_discount and transaction.order.coupon_discount > 0:
            ...
            if total_items > 0:
                coupon_discount_per_item = Decimal(
                    str(transaction.order.coupon_discount)) / total_items
